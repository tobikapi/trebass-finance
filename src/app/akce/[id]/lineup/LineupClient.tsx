...
interface Props {
  id: string
  initialArtists: LineupArtist[]
  initialContacts: Contact[]
  initialStages: string[]
  initialEventDates: { date: string | null; date_end: string | null }
}
...
export default function LineupClient({ id, initialArtists, initialContacts, initialStages, initialEventDates }: Props) {
  ...
  const [collapsedStages, setCollapsedStages] = useState<Record<string, boolean>>(
    () => Object.fromEntries(initialStages.map(s => [s, true]))
  )
    ...
    <EventLayout eventId={id}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <input
            value={search} onChange={e => setSearch(e.target.value)} placeholder="Hledat artista..."
          ...
          {stages.map((stageName, stageIdx) => {
            ...
            return (
              <div key={stageName} style={{ marginBottom: '36px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: isCollapsed ? 0 : '14px', padding: '12px 16px', borderRadius: '10px', backgroundColor: sc.bg, border: `1px solid ${sc.border}`, borderLeft: `4px solid ${sc.main}` }}>
                  <div onClick={() => setCollapsedStages(prev => ({ ...prev, [stageName]: !prev[stageName] }))} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', flex: 1 }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-faint)' }}>{stageArtists.length} {stageArtists.length === 1 ? 'artist' : 'artistů'}</span>
                  ...
                  {!multiDay && !isCollapsed && (
                    <button onClick={() => isStageFormOpen && !editId ? closeForm() : openForm(stageName)}
