...
export default function ElektrinaClient({ id, initialEquipment }: Props) {
  ...
  const groups: { key: string; label: EquipmentCategory | string; items: EventEquipment[] }[] = [
    ...EQUIPMENT_CATEGORIES.map(cat => ({ key: cat, label: cat, items: equipment.filter(e => e.category === cat) })),
    { key: NO_CATEGORY, label: 'Bez kategorie', items: equipment.filter(e => !e.category) },
  ...
  return (
    <EventLayout eventId={id}>
      ...
      <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 0, marginBottom: '20px' }}>
        Odběr (kW/ks) se zadává u jednotlivých položek na stránce Technika. Zde se sčítá napříč pronajímateli a místy, rozdělené po kategoriích.
      </p>
