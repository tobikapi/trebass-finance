...
export default function NovaAkcePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.error) {
      alert('Chyba při vytváření akce: ' + result.error)
      setLoading(false)
    } else if (result.data) {
  ...
  const inputStyle = {
    backgroundColor: '#0a0a0f', border: '1px solid #2a2a3e', color: '#f1f5f9',
    borderRadius: '8px', padding: '10px 14px', width: '100%', outline: 'none', fontSize: '14px',
  }
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="rounded-xl p-6 space-y-5" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading} className="px-6 py-2.5 rounded-lg text-sm font-medium" style={{ backgroundColor: '#7c3aed', color: '#fff', opacity: loading ? 0.6 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
            {loading ? 'Ukládám...' : 'Vytvořit akci'}
          </button>
          <button type="button" onClick={() => router.back()} className="px-6 py-2.5 rounded-lg text-sm font-medium" style={{ backgroundColor: '#1e1e2e', color: '#9ca3af' }}>
            Zrušit
          </button>
        </div>
      </form>
