...
const TYPES = ['umělec', 'dodavatel', 'venue', 'technika', 'security', 'jiné']
const TYPE_COLORS: Record<string, { color: string; bg: string }> = {
  umělec:    { color: '#f4978e', bg: '#2d1515' },
  dodavatel: { color: '#60a5fa', bg: '#1e2d3d' },
  venue:     { color: '#34d399', bg: '#052e16' },
  technika:  { color: '#a78bfa', bg: '#1e1535' },
  security:  { color: '#fb923c', bg: '#2d1a05' },
...
const emptyForm = { name: '', type: 'umělec', email: '', phone: '', note: '' }
...
export default function KontaktyPage() {
  ...
  function startEdit(c: Contact) {
    setForm({ name: c.name, type: c.type, email: c.email || '', phone: c.phone || '', note: c.note || '' })
        ...
        <form onSubmit={handleSave} style={{ marginBottom: '24px', padding: '20px', backgroundColor: '#161616', border: '1px solid #e05555', borderRadius: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div>
              ...
              <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Jméno umělce nebo firmy" style={inputStyle} />
            </div>
            <div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} placeholder="Rider, požadavky, jak ho známe..." style={inputStyle} />
          </div>
