...
export default function FirmaClient({ initialExpenses, initialIncome, initialContributions }: Props) {
  ...
  async function handleExpSave(e: React.FormEvent) {
    ...
    } else if (!editExpId && result.data) {
      const newId = (result.data as CompanyExpense).id
      pushUndo(`přidání firemního výdaje „${payload.item}“`, async () => {
        const res = await callAction('deleteCompanyExpense', newId)
        if (res.error) throw new Error(res.error)
        await load()
      })
    }
  ...
  async function handleToggleExpPaid(exp: CompanyExpense) {
    ...
    await callAction('toggleCompanyExpensePaid', exp.id, !prevPaid); await load()
    pushUndo(`změna platby „${exp.item}“`, async () => {
      const res = await callAction('toggleCompanyExpensePaid', exp.id, prevPaid)
      if (res.error) throw new Error(res.error)
      await load()
    })
  }
  ...
  async function handleIncSave(e: React.FormEvent) {
    ...
    } else if (!editIncId && result.data) {
      const newId = (result.data as CompanyIncome).id
      pushUndo(`přidání vkladu „${payload.name || payload.source}“`, async () => {
        const res = await callAction('deleteCompanyIncome', newId)
        if (res.error) throw new Error(res.error)
        await load()
      })
    }
      ...
      <div className="stat-grid" style={{ marginBottom: '32px' }}>
        {[
          ...
          <div key={s.label} style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
            ...
            <div style={{ fontSize: '22px', fontWeight: '700', color: s.color, marginTop: '4px' }}>
